...
# IF YOU WANT TO DEBUG, THE WORKFLOW CURRENTLY GOES LIKE THIS. THE EXCEL FILE IS READ IN AS THE DATAFRAME orth_df, AND THEN THE CORRECT "multi_species_homfinder_()" function is run on orth df, which finds all the correct orthologues for the input excel file/orth_df dataframe.
# After, restruc_df() function is run on orth_df and assigned to a new dataframe. restruc_df creates blank spaces between rows of a dataframe to allow for inserting future data
# Then we run the local_ens_id_finder() which translates all the ENSEMBL IDs to conventional gene names
# Lastly we run the pubcrawl() function on our dataframe, which gives the number of articles returned for the given search term + gene name on pubmed
...
#Uncomment these lines to test on my local laptop
...
def  multi_species_homfinder(orth_df, species_input):
    ...
    for i in tqdm(range(0, len(orth_df))):
        ...
        elif len(gentemp) == 1 and not namecount == homcount:
            orth_df.iloc[i, homcount] = gentemp[i][0]
...
#After you have run the python file, the program will ask for the path to the ensembl_gene_lists folder and the orthologous_gene_lists folders. These are required to find the orthologous genes and translate ensembl gene ids to conventional gene names
#Lastly, the program will ask for the path to your input gene list excel file. Please enter the path to the FOLDER CONTAINING YOUR INPUT FILE. This will be added to the working path
#After that, the file should run, and your output excel file should be in the same path as your input file, or whatever your current path is. i think. lol.
